...
export default class Service {
	...
	apis = {
		LOGIN: 					`${this.#api_path}/auth/login`,
		LOGOUT: 				`${this.#api_path}/auth/logout`,
		SESSION: 				`${this.#api_path}/auth/session`,

		CREATE: 				`${this.#api_path}/user`,
		USERS: 					`${this.#api_path}/user`,
		USER: 					`${this.#api_path}/user/{{id}}`,
		COOKIE_CONSENT: 		`${this.#api_path}/user/cookieConsent`,
		SEND_MAIL: 				`${this.#api_path}/user/sendMail`,
		...
		ORDERS: 				`${this.#api_path}/orders`,
		ORDERS_BY_STATUS: 		`${this.#api_path}/orders/status/{{status}}`,
		ORDER: 					`${this.#api_path}/orders/{{id}}`,
		ORDER_STATUSES: 		`${this.#api_path}/orders/statuses`,

		REPORT_ORDER_STATS: 	`${this.#api_path}/report/orderStats`,
		REPORT_TOP_PRODUCTS: 	`${this.#api_path}/report/topProducts`,
		REPORT_RECENT_ORDERS: 	`${this.#api_path}/report/recentOrders`
	};
	...
	async get(url, data, options, successCB, errorCB) {
		...
		if (response.ok) {
			if (successCB) successCB(response.responseData, response);
		} else {
			if (errorCB) errorCB(response.statusText, response);
		}
	}
