...
class Router {
	...
	#initRoutes() {
		...
		this.on('sign-up', () => this.#handler(false, 'gotoSignup'));
		...
		this.on('orders', () => this.#handler(true, 'gotoOrders'));
		this.on('orders/:status', (params) => this.#handler(true, 'gotoOrders', params));
		this.on('order/:id', (params) => this.#handler(true, 'gotoOrder', params));
		this.on('contact-us', () => this.#handler(false, 'gotoContactUs'));
		
		this.on('refresh', () => this.#handler(false, 'gotoPageRefresh'));
		this.on('privacy', () => this.#handler(false, 'gotoPolicyPrivacy'));
		this.on('terms', () => this.#handler(false, 'gotoPolicyTerms'));
		this.on('cookies', () => this.#handler(false, 'gotoPolicyCookies'));
		this.on('cookie-preferences', () => this.#handler(false, 'gotoCookiePreferences'));

		this.on('pagenotfound', () => this.#handler(false, 'gotoPageNotFound'));

		// protected routes:
		this.on('my-account', () => this.#handler(true, 'gotoMyAccount'));
		this.on('my-account/edit-profile', () => this.#handler(true, 'gotoEditProfile'));
		this.on('my-account/change-password', () => this.#handler(true, 'gotoChangePassword'));
