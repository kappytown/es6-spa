...
export default class ProductsView extends View {
	...
	constructor() {
		...
		this.model.setSuccessListener(this.showProducts.bind(this));
					...
					this.model.fetchCategories((categories) => {
						if (categories?.data?.length > 0) {
							...
							categories.data.forEach(item => {
								...
								$category.append(`<option value="${item.category}" ${selected}>${item.category}</option>`);
							});
						}
					});
				}
	...
	showProducts(products, currentPage, totalPages) {
		...
		if (products?.data?.length > 0) {
			// Render the template
			$el.append(this.templateManager.getContent('partials/products_list.html', { products: products.data }));
